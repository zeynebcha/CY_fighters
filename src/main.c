...
int main(void){
  ...
  if (mode_jeu==1){ //mode joueur vs machine
    ...
    if(dif >= 1 && dif <= 3){// équipe faible prédéfinie
      ...
      team_md1 = rand()%3 + 1;
      if(team_md1 == 1){
        printf("Le joueur va jouer contre l'équipe prédéfinie 1 contenant  Miroveed, Nallmar, Daera, Lorel\n");
        ...
        printf("Le joueur va jouer contre l'équipe prédéfinie 2 contenant Arel, Tagriel , Iela et Arrion\n");
      ...
      if(team_md1== 3){
        printf("Le joueur va jouer contre l'équipe prédéfinie 3 contenant Deam, Daera, Tagriel et Lorel\n");
